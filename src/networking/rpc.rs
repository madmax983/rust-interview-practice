...
impl RpcRequest {
    ...
    fn parse<R: Read>(reader: &mut BufReader<R>) -> io::Result<Option<Self>> {
        // ⚡ BOLT OPTIMIZATION: Reuse a single String buffer for all lines to avoid
        // multiple heap allocations per request parse.
...
impl RpcResponse {
    ...
    fn parse<R: Read>(reader: &mut BufReader<R>) -> io::Result<Option<Self>> {
        // ⚡ BOLT OPTIMIZATION: Reuse a single String buffer for all lines to avoid
        // multiple heap allocations per response parse.
        let mut buf = String::new();
        if reader.read_line(&mut buf)? == 0 {
            return Ok(None); // EOF
        }
