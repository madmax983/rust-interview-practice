...
impl<'a> Lexer<'a> {
    ...
    // ⚡ BOLT OPTIMIZATION: Replaced intermediate `String::new()` allocations and
    // character-by-character `.push()` loops with direct string slicing (`&self.input[start_pos..self.pos]`).
    // This eliminates intermediate heap allocations (amortized O(log N) due to capacity doubling) during tokenization, reducing allocation overhead.
