...
//! **Dynamic Programming**
//!
//! 1.  **Straightforward 2D DP Table**: We create a 2D matrix `dp` of size `(M+1) x (N+1)`, where `M` and `N` are
...
//!
//! 2.  **Optimized 1D DP Array**: Notice that `dp[i][j]` only depends on the current row `dp[i][...]` and
...
//!
//! - **Time Complexity**: `O(M * N)` where `M` and `N` are the lengths of `text1` and `text2`. We must evaluate
//!   each pair of characters.
//! - **Space Complexity**:
//!   - Straightforward: `O(M * N)` for the 2D DP table.
//!   - Optimized: `O(min(M, N))` by storing only a single row (or two rows) of DP state, caching the smaller string.

use std::cmp;

/// Straightforward 2D DP Table Approach
///
/// Builds a full `(M+1) x (N+1)` matrix to store overlapping subproblems.
#[must_use]
#[allow(clippy::needless_pass_by_value)]
pub fn longest_common_subsequence_straightforward(text1: String, text2: String) -> i32 {
...
/// Optimized 1D DP Array Approach
///
...
mod tests {
    ...
    #[test]
    fn test_straightforward_happy_path() {
        assert_eq!(
            longest_common_subsequence_straightforward("abcde".to_string(), "ace".to_string()),
            3
        );
        assert_eq!(
            longest_common_subsequence_straightforward("abc".to_string(), "abc".to_string()),
            3
        );
    }
