        ...
        loop {
            ...
            if bytes_read == 0 {
                ...
                if !row.is_empty() || !self.field_buf.is_empty() || in_quotes {
                    ...
                    // ⚡ BOLT OPTIMIZATION: Use `std::mem::take` to avoid allocating a new String and copying contents, replacing `.clone()` overhead.
            ...
            while let Some(c) = chars.next() {
                if in_quotes {
                    ...
                    if c == self.config.quote {
                        ...
                        // End of field
                        // PRODUCTION NOTE: We use `std::mem::take` here to avoid lifetime issues
                        // with yielding references to an internal buffer, and to prevent an
                        // O(N) string allocation and copy per field parsed!
                        row.push(std::mem::take(&mut self.field_buf));
                    } else if c == '\r' || c == '\n' {
                        // End of line.
                        ...
                        // We finish the row
                        // ⚡ BOLT OPTIMIZATION: Avoid `.clone()` overhead for the final field of the line.
                        row.push(std::mem::take(&mut self.field_buf));
                        return Some(Ok(row));
                    } else {
                        self.field_buf.push(c);
            ...
            if !in_quotes {
                // ⚡ BOLT OPTIMIZATION: Use `std::mem::take` to avoid trailing clone.
                row.push(std::mem::take(&mut self.field_buf));
                self.eof = true;
                return Some(Ok(row));
            }
