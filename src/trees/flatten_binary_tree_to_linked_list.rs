...
pub fn flatten_brute_force(root: Option<Rc<RefCell<TreeNode>>>) {
    ...
    fn pre_order(node: Option<Rc<RefCell<TreeNode>>>, nodes: &mut Vec<Rc<RefCell<TreeNode>>>) {
        if let Some(n) = node {
            ...
            pre_order(left, nodes);
            pre_order(right, nodes);
        }
    }

    pre_order(root.clone(), &mut nodes);
...
mod tests {
    ...
    fn build_test_tree() -> Option<Rc<RefCell<TreeNode>>> {
        ...
        n2.as_ref().unwrap().borrow_mut().left = n3.clone();
        n2.as_ref().unwrap().borrow_mut().right = n4.clone();

        n5.as_ref().unwrap().borrow_mut().right = n6.clone();

        root.as_ref().unwrap().borrow_mut().left = n2.clone();
        root.as_ref().unwrap().borrow_mut().right = n5.clone();
