...
mod tests {
    ...
    fn test_happy_path() {
        ...
        // Test Brute Force
        let mut root_bf = Some(root.clone());
    ...
    fn test_single_node() {
        ...
        flatten_brute_force(&mut root1);
        assert_eq!(to_vec(&root1), vec![Some(0)]);

        let mut root2 = Some(TreeNode::new(0));
        flatten_optimal(&mut root2);
        assert_eq!(to_vec(&root2), vec![Some(0)]);
    }
    ...
    fn test_left_heavy_tree() {
        ...
        let mut root_opt = Some(root);
        flatten_optimal(&mut root_opt);
        assert_eq!(to_vec(&root_opt), vec![Some(1), Some(2), Some(3)]);
    }
}
