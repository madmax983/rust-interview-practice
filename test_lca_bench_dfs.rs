...
pub fn lowest_common_ancestor_optimized_new(
    ...
    let p_ptr = Rc::as_ptr(p.as_ref().unwrap());
    let q_ptr = Rc::as_ptr(q.as_ref().unwrap());

    fn dfs(
        node: Option<Rc<RefCell<TreeNode>>>,
        p_ptr: *const RefCell<TreeNode>,
        q_ptr: *const RefCell<TreeNode>,
    ) -> Option<Rc<RefCell<TreeNode>>> {
        let node = node?;

        // Compare raw pointers instead of `Rc::ptr_eq` for speed
        if std::ptr::eq(Rc::as_ptr(&node), p_ptr) || std::ptr::eq(Rc::as_ptr(&node), q_ptr) {
            return Some(node);
        }

        let left = dfs(node.borrow().left.clone(), p_ptr, q_ptr);
        let right = dfs(node.borrow().right.clone(), p_ptr, q_ptr);
    ...
    dfs(root, p_ptr, q_ptr)
}

pub fn lowest_common_ancestor_optimized_ref(
    root: Option<Rc<RefCell<TreeNode>>>,
    p: Option<Rc<RefCell<TreeNode>>>,
    q: Option<Rc<RefCell<TreeNode>>>,
) -> Option<Rc<RefCell<TreeNode>>> {
...
fn main() {
    ...
    println!("New (ptr): {:?}", start.elapsed());

    let start = Instant::now();
    for _ in 0..1000 {
        std::hint::black_box(lowest_common_ancestor_optimized_ref(Some(root.clone()), Some(p.clone()), Some(q.clone())));
    }
    println!("New (ref): {:?}", start.elapsed());
}
